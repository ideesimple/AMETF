...
$('#AffPass').click(function() {
		if ($(this).is(':checked')) {
			$('#motdepasse').replaceWith($('#motdepasse').clone().attr('type', 'text').attr('AUTOCOMPLETE', 'OFF'));
			$('#motdepasse2').replaceWith($('#motdepasse2').clone().attr('type', 'text').attr('AUTOCOMPLETE', 'OFF'))


		} else {
			$('#motdepasse').replaceWith($('#motdepasse').clone().attr('type', 'password'));
			$('#motdepasse2').replaceWith($('#motdepasse2').clone().attr('type', 'password'))
		}
	});
...
$(".comparateurPrix table td").filter(':nth-child(3)').each(function() {
    ...
    if ($(this).find("div").length >1) {
        ...
        $(this).next().find("span.text").append(picto).bind('click',function() {
                    if ($(this).parent().find('div.blocChoixDevis').css('display')=="block") {
                        $(this).parent().find('div.blocChoixDevis').css('display','none')
                    }
                    else {
                        $(this).parent().find('div.blocChoixDevis').css('display','block')
                    };
                        $(this).parent().find('input.chinput').focus();
...
maxHeight=0;
    $(".memeHeight_Infoex").each(function () {
        height= ($.browser.webkit == true) ?  $(this).outerHeight()+10:$(this).height();
