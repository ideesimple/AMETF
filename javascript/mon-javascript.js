...
maxHeight=0;
    $(".memeHeight_Infoex, .blocAvecFdecebebSshomeB2").each(function () {
        $(this).css("height",'');
        var clone=$(this).clone();
        var blocTest= $("<div id='testwidth11'></div>").css({
        position:"absolute",
        visibility:"hidden",
        display:"block"
        });
...
//        var w = $("#testwidth11").width();
		var h = $("#testwidth11").height();
        maxHeight = (h > maxHeight) ? h : maxHeight;
$("#testwidth11").remove()

    })
    $(".memeHeight_Infoex").css("height",maxHeight-11);

}

function memhauteur1 () {
var maxHeight2=0;

$(".memhauteur1").each(function () {
        $(this).css("height",'');
        var clone=$(this).clone();
        lesclass=$(this).attr("class");
        var blocTest= $("<div id='testwidth12' class='blocText'></div>").css({
        position:"absolute",
        visibility:"hidden",
        display:"block"
        });
        ...
        $("body").append(blocTest);
//        var w = $("#testwidth11").width();
		var h = $("#testwidth12").height();
        maxHeight2 = (h > maxHeight2) ? h : maxHeight2;
$("#testwidth12").remove()
    })
    $(".memhauteur1").css("height",maxHeight2-12);
}




function memhauteur2 () {
var maxHeight3=0;

$(".memhauteur2").each(function () {
        $(this).css("height",'');
        var clone=$(this).clone();
        var blocTest= $("<div id='testwidth13' class='blocText'></div>").css({
        position:"absolute",
        visibility:"hidden",
        display:"block"
        });
        blocTest.append(clone);
        $("body").append(blocTest);
		var h = $("#testwidth13").height();
        maxHeight3 = (h > maxHeight3) ? h : maxHeight3;
$("#testwidth13").remove()
    })
    $(".memhauteur2").css("height",maxHeight3-12);
}
...
function blocSelectPart() {
var dimElem=jQuery.iUtil.GetDimElemH1($("h1 span"));
$(".blocSelectPart").css ({
"left":dimElem.w+40,
"top":15
})
...
blocSelectPart();
//$("#testCoMo").html($("#testCoMo").html().replace(/[Ergebnisverwendung]+/, 'Ergebnis- verwendung'))
//txtacouper=$(".replaceText").html();
//txtacouper=txtacouper.replace(/[he]+/, 'he<br />');
//$(".replaceText").html(txtacouper);


$(".comparateurPrix table td.orange span.text a, .BLocResultSearch table span.orange a").each(function () {
    $(this).bind("click",function () {
            $("#BLocACacher").css('display','none');
            $("#BLocAMontrer").css('display','block');
            $(".LienRevSearch").css("visibility","visible");
            blocSelectPart();
            memeHeight_Infoex();
...
$("#les_plus_consultes ul li a").each(function () {
    $(this).bind("click",function () {
            $("#BLocACacher").css('display','none');
            $("#BLocAMontrer").css('display','block');
            blocSelectPart();
            memeHeight_Infoex();
            $(".LienRevSearch").css("visibility","hidden");
            $('html,body').animate({scrollTop: $("#columns").offset().top},'slow');
        return false;
       }
    )
...
$(".AccesAuFonds > div > div > div > div > div a").live("click",function () {
...
$("#ReturnRecherche").each(function () {
    $(this).bind("click",function () {


/*        $("#BLocAMontrer").hide();
        $("#BLocACacher").show(function () {
         blocSelectPart();
            memeHeight_Infoex();
        });
        return false;
...
(function($){
 $.fn.pagination = function(options) {
    ...
    return this.each(function() {
        ...
        Aajouter=queltableau.parent().find('table.paginated tbody tr.Aajouter').length;
        if (Aajouter>0) {
        queltableau.find('tbody tr').hide().show();
        queltableau.find('tbody tr:gt('+parseFloat(opts.pagenumber*(parseFloat(opts.numParPage)+Aajouter) - 1)+')').hide().end();
        }
    ...
    $("#tablPageCheaper .paginated").pagination({ pagenumber: 1,numParPage:20, buttonClickCallback: PageClick});
        ...
        PageClick = function(pageclickednumber,queltableau,numParPage) {
            ...
            queltableau.parent().find('table.paginated tbody tr:lt('+parseFloat((pageclickednumber-1) * numParPage)+')').hide().end();
                    ...
                    Aajouter=queltableau.parent().find('table.paginated tbody tr.Aajouter').length;
                if (Aajouter>0) {
                queltableau.parent().find('table.paginated tbody tr:gt('+parseFloat((pageclickednumber) * (numParPage+Aajouter) - 1 )+')').hide().end();
                }
...
(function($){
    ...
    $(".customStyleSelectBox").remove();
            return this.each(function() {
                ...
                var DimSelect=jQuery.iUtil.GetDimSelect($(this));
                var selectBoxWidth = parseInt(DimSelect.w) - parseInt(selectBoxSpan.css('padding-left')) -parseInt(selectBoxSpan.css('padding-right'));
                ...
                longueur=jQuery.iUtil.GetDimElem($(this).next().find('.blocOption'));
                           $(this).next().find('.customStyleSelectBoxInner').css('width',longueur.w+10);
                $(this).next().find('.blocOption').css('width',longueur.w+10);
...
jQuery.iUtil = {
	...
	GetDimElem : function(e)
	{
        var clone=e.clone();
        var blocTest= $("<div id='testDim'></div>").css({
        position:"absolute",
        visibility:"hidden",
        display:"block"
        });
        ...
        $("body").append(blocTest);
        var w = ($("#testDim").width());
		var h = $("#testDim").height();
        $("#testDim").remove();
		return {w:w, h:h};
	},
	GetDimElemH1 : function(e)
	{
        var clone=e.clone();
        var blocTest= $("<div style='background-color:red' id='testDimH1' class='h1SPan'></div>").css({
        position:"absolute",
        display:"block"
        });
        blocTest.append(clone);
        $("body").append(blocTest);
        var w = $("#testDimH1").width();
		var h = $("#testDimH1").height();
        $("#testDimH1").remove();
		return {w:w, h:h};
	},
	GetDimSelect : function(e)
	{
        var clone=e.clone();
        var blocTest= $("<div id='testDim'></div>").css({
        position:"absolute",
        ...
        $("body").append(blocTest);
           ...
           if($.browser.msie&&$.browser.version <8) {
            var w = ($("#testDim select").width()+27);
    		var h = $("#testDim select").height();
          }
          else {
            var w = ($("#testDim select").width()+27);
    		var h = $("#testDim select").height();
